...
class TAREEngine:
    ...
    def inject_fault(self, zone_id: str, fault_msg: str):
        """Introduce a fault into a zone — used to simulate fault before repair scenario."""
        with self._lock:
...
Write a 3-4 sentence briefing for the supervisor. Include: what the agent did, why it is suspicious given its work order, what TARE has done in response, and what decision the supervisor must make (approve a 3-minute time-box or deny and escalate). Be specific about zones and commands. Use the word "breached" (not "attacked") when referring to zones accessed outside the work order. Do not use bullet points."""
    ...
    # ─── Priority based on highest-risk zone involved ───────────────────────────
    def _incident_priority(self, signals=None):
        """
        P1 Critical  — identity breach, burst-rate, or high-impact attack on a healthy zone
        P2 High      — quiet out-of-zone escalation or ML-only recon (read-only, no burst)
        P3 Medium    — Z3-only, authorised zone, low-impact
        """
